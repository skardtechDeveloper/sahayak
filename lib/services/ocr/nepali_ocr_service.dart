...
class NepaliOCRService {
  ...
  Future<void> initialize() async {
    ...
    try {
      _interpreter = await Interpreter.fromAsset('assets/models/nepali_ocr.tflite');
    } catch (e) {
  ...
  Future<String> _recognizeWithCustomModel(img.Image image) async {
    ...
    return _decodeModelOutput(output);
  }
  ...
  List<List<double>> _prepareImageForModel(img.Image image) {
    // Resize to 128x128
    ...
    for (var y = 0; y < 128; y++) {
      for (var x = 0; x < 128; x++) {
        ...
        final gray = (img.getRed(pixel) + img.getGreen(pixel) + img.getBlue(pixel)) / 3.0;
